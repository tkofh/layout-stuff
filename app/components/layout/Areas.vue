<template>
  <LayoutPrimitive :as class="layout-areas" :style :data-scroll-direction="direction">
    <RadixSlot v-if="slots.top" ref="top" class="layout-area" :data-layout-area="topArea">
      <slot name="top" />
    </RadixSlot>
    <RadixSlot v-if="slots.left" ref="left" class="layout-area" :data-layout-area="leftArea">
      <slot name="left" />
    </RadixSlot>
    <RadixSlot v-if="slots.right" ref="right" class="layout-area" :data-layout-area="rightArea">
      <slot name="right" />
    </RadixSlot>
    ...
    <RadixSlot v-if="slots.bottom" ref="bottom" class="layout-area" :data-layout-area="bottomArea">
      <slot name="bottom" />
    </RadixSlot>
  </LayoutPrimitive>
</template>
...
const style = computed(() => ({
  ...
  '--areas-size-right': rightSize.value !== 0 ? `${rightSize.value}px` : undefined,
  '--areas-size-bottom': bottomSize.value !== 0 ? `${bottomSize.value}px` : undefined,
...
@layer layout.init {
  .layout-areas {
    ...
    &[data-scroll-direction='vertical'] > .layout-sticky[data-layout-area~='x'] {
      &:not([data-layout-area~='start']) {
        --sticky-cross-clip-start: calc(var(--areas-size-top) - var(--scroll-start));
      }

      &:not([data-layout-area~='end']) {
        --sticky-cross-clip-end: calc(var(--areas-size-bottom) - var(--scroll-end));
      }
    ...
    &[data-scroll-direction~='horizontal'] > .layout-sticky[data-layout-area~='y'] {
      &:not([data-layout-area~='start']) {
        --sticky-cross-clip-start: calc(var(--areas-size-left) - var(--scroll-start));
      }

      &:not([data-layout-area~='end']) {
        --sticky-cross-clip-end: calc(var(--areas-size-right) - var(--scroll-end));
      }
