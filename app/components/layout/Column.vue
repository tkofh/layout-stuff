...
import { useCollapseBelow, type CollapseBelow } from '~/components/layout/internal/Collapsible.vue'
...
function columnStyle(
  ...
  const result = {
    ...responsiveToAttributes(
      '--inline-size',
      compactResponsive(
        mapResponsive(fillResponsive(normalizeResponsive(width)), (value, breakpoint) => {
          if (
            collapseBelow !== 'none' &&
  ...
  if (collapseBelow !== 'none') {
    Object.assign(
      ...
      responsiveToAttributes(
        ...
        mapResponsive(normalizeResponsive(columnsAlign), (value, breakpoint) => {
          if (smallerBreakpoint(breakpoint, collapseBelow) === collapseBelow) {
            return undefined
          }
...
const width = computed(() => fillResponsive(normalizeResponsive(props.width ?? 'fluid')))
...
const align = computed(() =>
  compactResponsive(
    mapResponsive(fillResponsive(normalizeResponsive(columnsAlign.value)), (value, breakpoint) => {
      if (
        collapseBelow.value !== 'none' &&
        collapseBelow.value !== breakpoint &&
...
@layer layout.component {
  .layout-column {
    ...
    @container style(--screen-lt-tablet: true) {
    ...
    @container style(--screen-lt-laptop: true) {
    ...
    @container style(--screen-lt-desktop: true) {
