...
export type ChildrenMapper<I, O> = (children: ReadonlyArray<Child<I, O>>) => void
...
class Child<I, O> {
  ...
  *after() {
    ...
    while (current != null) {
      for (const ref of current.siblings.slice(current.siblings.indexOf(current) + 1)) {
        yield ref
      }
  ...
  compare(other: Child<I, O> | undefined): boolean {
    return other != null && this.uid === other.uid && this.input === other.input
  }
}
    ...
    watch(
      ...
      () => {
        ...
        for (const value of walkChildComponents(
          this.#vm.vnode,
          (vnode) => vnode !== this.#vm.vnode && vnode.component?.type !== this.#vm.type,
        )) {
...
export function useChildren<I, O = never>(channel: string, handler: ChildrenMapper<I, O>) {
  ...
  provide(Symbol.for(`use-children:${channel}`) as InjectionKey<ChildrenContext<I, O>>, context)
}
...
export function useChild<I, O = never>(
  ...
  const context = inject(
    Symbol.for(`use-children:${channel}`) as InjectionKey<ChildrenContext<I, O>>,
    null,
  )
