...
export type BreakpointsExcept<B extends BreakpointName> = Exclude<BreakpointName, B>
...
> = V extends PartialResponsiveMap<infer T> ? T : V extends SparseResponsiveMap<infer T> ? T : V
...
export const breakpoints = new Set(['mobile', 'tablet', 'laptop', 'desktop'] as const)

export function smallerBreakpoint(a: BreakpointName, b: BreakpointName): BreakpointName {
...
export function isSparseResponsiveMap<V>(value: unknown): value is SparseResponsiveMap<V> {
...
export function normalizeResponsive<const V extends ResponsiveValue<unknown>>(
  ...
  if (isSparseResponsiveMap(input)) {
    return {
      mobile: input.mobile,
      tablet: input.tablet == null || input.tablet === input.mobile ? null : input.tablet,
      laptop: input.laptop == null || input.laptop === input.tablet ? null : input.laptop,
      desktop: input.desktop == null || input.desktop === input.laptop ? null : input.desktop,
    } as never
  }
...
export function mapResponsive<V extends ResponsiveMap<unknown>, M extends ValueMapper<V>>(
  input: V,
  mapper: M,
): ResponsiveMap<ReturnType<M>>
export function mapResponsive<V extends PartialResponsiveMap<unknown>, M extends ValueMapper<V>>(
  input: V,
  mapper: M,
...
export function mapResponsive<V>(
  ...
  return {
    ...
    tablet: input.tablet === null ? null : mapper(input.tablet as Exclude<V, null>, 'tablet'),
    laptop: input.laptop === null ? null : mapper(input.laptop as Exclude<V, null>, 'laptop'),
    desktop: input.desktop === null ? null : mapper(input.desktop as Exclude<V, null>, 'desktop'),
  }
}
